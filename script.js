...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Event Listeners for Module compatibility
...
async function addNewRide() {
    ...
    try {
        ...
        document.getElementById('ride-title').value = ''.trim();
        document.getElementById('ride-date').value = ''.trim();
    } catch (e) {
        ...
        snapshot.forEach((doc) => {
            ...
            container.innerHTML += `
                <div style="background:#f9f9f9; padding:10px; border-radius:8px; margin-top:8px; border:1px solid #eee;">
                    ...
                    <button onclick="window.joinRide('${escapeHtml(ride.title)}')" style="padding:6px 12px; font-size:0.85rem; margin-top:6px; width:auto;">Συμμετοχή</button>
                </div>
            `;
        });
    });
...
window.joinRide = function(title) {
